    ...
    */
    ...
    DayGrid.mixin({
        ...
        fgSegHtml: function(seg, disableResizing) {
            ...
            channelHtml = '<i class="fa fa-fw fa-' + eventIcon + '"></i>';
            ...
            var imgurl = htmlEscape(event.attachment);
            var quickview = '';
            if (imgurl != '') {
                quickview = '<span class="fc-tooltiptext uk-box-shadow-small"><img src="' + imgurl + '" alt="' + tooltipText + '" /><br>' + tooltipText + '</span>';
            } else {
                quickview = '<span class="fc-tooltiptext uk-box-shadow-small">' + tooltipText + '</span>';
            }

            var content = tooltipText;
            if(tooltipText.length>16){
                content = jQuery.trim(tooltipText).substring(0, 16).trim(this) + "...";
            }

            return '<a class="fc-tooltip color-text-' + htmlEscape(event.channel) + ' ' + classes.join(' ') + '"' +
                (event.url ?
                ...
                (this.isRTL ?
                    channelHtml + ' ' + content + ' ' + timeHtml + ' ' : // put a natural space in between
                    ' ' + timeHtml + ' ' + channelHtml + ' ' + content //
                ) +
                '</div>' +
    ...
    var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
        ...
        buildGotoAnchorHtml: function(gotoOptions, attrs, innerHtml) {
            ...
            if (!forceOff && this.opt('navLinks')) {
                var now = FC.moment();
                var d = FC.moment(date).format();
                var add = baseUrl+'/engagement/add?date='+d;
                var day = '<a' + attrs + ' data-goto="' + htmlEscape(JSON.stringify(finalOptions)) + '" title="View this day" uk-tooltip>' + innerHtml + '</a>';
                if (date > now) {
                    return day + '<a href="' + htmlEscape(add) + '" class="fa fa-plus fc-addnew grey-text" title="Add New" uk-tooltip></a>';
                } else {
                    return day;
                }
            } else {
            ...
            if (buttonStr) {
                $.each(buttonStr.split(' '), function(i) {
                        ...
                        if (buttonName == 'title') {
                            ...
                            if (buttonClick) {
                                ...
                                button = $( // type="button" so that it doesn't submit a form
                                        '<button type="button" class="uk-button uk-button-secondary uk-button-small ' + classes.join(' ') + '">' +
                                        innerHtml +
                                        '</button>'
                                    )
                                    .click(function(ev) {
                    ...
                    if (groupChildren.length > 1) {
                        groupEl = $('<div/>');
                        if (isOnlyButtons) {
                            groupEl.addClass('fc-button-group uk-button-group ');
                        }
